...
from keras.layers import DepthwiseConv2D, MaxPooling3D, MaxPooling1D, Conv1D, RepeatVector
from keras.layers import Lambda, Dot, Multiply, Input, Permute, LSTM, Dense, TimeDistributed 
from keras.layers import Conv2D, ConvLSTM2D, Flatten, Reshape, BatchNormalization
...
class SP(SP):
    def time_dist(self,data):
        x = np.zeros((data.shape[0]-TIME_DIM+1,TIME_DIM,data.shape[1]), dtype='float')
...
class SP(SP):
    def build_model(self):
        ...
        '''
        '''
        noise = Input(shape=(self.latent_dim), name='G_Input')
        l1 = Reshape((noise.shape[1],noise.shape[2],1), name='Reshape_s1s21')(noise)
        l2 = Reshape((noise.shape[1],1,noise.shape[2]), name='Reshape_s11s2')(noise)
        aff = Lambda(lambda x: tf.matmul(x[0],x[1]), name='Affinity')([l1,l2])
        aff = Reshape((TIME_DIM,noise.shape[2],noise.shape[2],1), name='Affinity_t_sliced')(aff)
        seq1 = ConvLSTM2D(32, 3, activation = self.hl_activation,return_sequences=True)(aff)
        seq2 = ConvLSTM2D(32, 3, activation = self.hl_activation,return_sequences=True)(seq1)
        seq3 = ConvLSTM2D(32, 3, activation = self.hl_activation,return_sequences=True)(seq2)
        seq = Reshape((1,TIME_DIM,seq3.shape[2]*seq3.shape[3]*seq3.shape[4]))(seq3)
        per = Permute((1,3,2))(seq)
        wgh = DepthwiseConv2D(1,1,activation = 'softmax')(per)
        aff = Multiply()([wgh,per])
        mean = Lambda(lambda x: K.mean(x, axis=3))(aff)
        per2 = Permute((2,1))(mean)
        conv4 = Conv1D(16,3, activation = self.hl_activation)(per2)
        bn4 = BatchNormalization()(conv4)
        pool1 = MaxPooling1D(2)(bn4)
        conv5 = Conv1D(32,3, activation = self.hl_activation)(pool1)
        bn5 = BatchNormalization()(conv5)
        pool2 = MaxPooling1D(2)(bn5)
        conv6 = Conv1D(64,3, activation = self.hl_activation)(pool2)
        bn6 = BatchNormalization()(conv6)
        pool3 = MaxPooling1D(2)(bn6)
        conv6 = Conv1D(128,3, activation = self.hl_activation)(pool2)
        bn6 = BatchNormalization()(conv6)
        pool3 = MaxPooling1D(2)(bn6)
        pool3 = Flatten()(pool3)
        dense2 = Dense(50, activation = self.hl_activation)(pool3)
        dense3 = Dense(self.f_cols, activation = self.o_activation)(dense2)
        model = Model(inputs=noise,outputs=dense3)
        model.summary()
        return model
        '''
...
class SP(SP):
    def train(self, epochs, batch_size):        
        ...
        np.savetxt('results/train_3/pred_{0}.csv'.format(epochs), Y_pred, delimiter=',')
        np.savetxt('results/train_3/rmse_{0}.csv'.format(epochs), rms_val, delimiter=',')
        plt.plot(list(range(0, Y_pred.shape[0])),Y_pred[:,3],'-',color='red')
        plt.plot(list(range(0, Y_train.shape[0])),Y_train[:,3],'-',color='blue')
        plt.savefig('results/train_3/plot_{0}.png'.format(epochs))
        plt.close()
